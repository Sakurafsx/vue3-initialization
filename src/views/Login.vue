...
async function handleLogin() {
  ...
  try {
    ...
    // 实际的登录API调用
    const response: any = await login({ username: safeUsername, password: safePassword })
    ...
    errorMessage((error as any)?.message ?? "登录失败，请稍后重试")
  }
}
    ...
    <div class="login-container">
      ...
      <form @submit.prevent="handleLogin" class="floating-form">
        ...
        <div class="form-footer">
          <span>还没有账号？</span>
          <a @click.prevent="router.push({ name: 'Register' })">立即注册</a>
        </div>
      </form>
    </div>
  </div>
