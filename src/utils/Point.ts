...
class Graph {
  ...
  constructor(nums: number = 30, public maxDistance: number = 150, cvs: HTMLCanvasElement, ctx: CanvasRenderingContext2D) {
    this.points = new Array(nums).fill(0).map(() => new Point(cvs, ctx));
  ...
  draw() {
    ...
    for (let i = 0; i < this.points.length; i++) {
      ...
      for (let k = i + 1; k < this.points.length; k++) {
        const d = Math.sqrt(Math.pow(this.points[i].x - this.points[k].x, 2) + Math.pow(this.points[i].y - this.points[k].y, 2));
...
export { Point, Graph, ContentCanvas };
